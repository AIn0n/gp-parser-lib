...
from parsers.grammars.example_grammars import GRAMMAR_3_20, GRAMMAR_3_23
...
def test_goto_given_grammar_3_20_returns_valid_set() -> None:
    ...
    expected_items = [
        LRItem.from_rule_str("S -> ( L )", dot_pos=1, eol="$end"),
        LRItem.from_rule_str("L -> S", dot_pos=0, eol="$end"),
        LRItem.from_rule_str("L -> L , S", dot_pos=0, eol="$end"),
        LRItem.from_rule_str("S -> ( L )", dot_pos=0, eol="$end"),
        LRItem.from_rule_str("S -> x", dot_pos=0, eol="$end"),
    ]
    p = LR0Parser(appel_to_ruleset(GRAMMAR_3_20))
...
def test_lr0_for_grammar_3_23_returns_duplicate_entry_in_parsing_table() -> None:
    ...
    p = LR0Parser(appel_to_ruleset(GRAMMAR_3_23))
